...
import { PrivateOrdersSection } from "@/modules/app/routes/shipper/pages/private/pages/orders/section/private-orders-section"

export function PrivateOrdersView({ path, search, cargoType }: { path: ORDERS_PATH, search?: string, cargoType?: string }) {
    return (
        <Suspense fallback={<OrdersLoadingFallback />} >
            <ErrorBoundary fallback={<OrdersErrorFallback />} >
                ...
                <PrivateOrdersSection path={path} search={search} cargoType={cargoType} />
            </ErrorBoundary>
        </Suspense>
    )
}
