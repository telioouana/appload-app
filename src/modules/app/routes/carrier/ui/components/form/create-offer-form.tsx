...
import { OfferSchemaForm } from "../../../schemas/offer"
...
export function CreateOfferForm({ values }: Props) {
    const [drivers, setDrivers] = useState<Driver[] | []>([])
    const [fleet, setFleet] = useState<Fleet[] | []>([])
    const [driver, setDriver] = useState<string>("")
    const [plate, setPlate] = useState<string>("")

    const t = useTranslations("Carrier.offer.create.form")

    const { control, formState: { isSubmitting }, setValue } = useFormContext<OfferSchemaForm>()
    ...
    function handleSelectFleet(value: Fleet) {
        ...
        if (value.trailer) {
            ...
            setValue("type", value.trailer.loadingBay.type as typeof LOADING_BAY[number])
        } else {
