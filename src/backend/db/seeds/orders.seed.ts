...
import { order } from "@/backend/db/schema";
...
export async function seedOrders() {
  for (const o of ORDERS) {
    // omit id and legacyId so Postgres defaults apply
    await db.insert(order).values(o);
  }
  console.log(`Inserted ${ORDERS.length} orders for org ${ORG_ID}`);
}
